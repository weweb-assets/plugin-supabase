...
export default {
    ...
    methods: {
        ...
        async loadBranches(env) {
            console.info('[Supabase plugin] loadBranches start', { env, hasOAuth: this.hasOAuthToken(), projectUrl: this.getCurrentEnvConfig(env).projectUrl });
            try {
                ...
                console.info('[Supabase plugin] loadBranches success', { env, branches: (data?.data || []).map(b => b?.name), count: data?.data?.length || 0 });
            } catch (e) {
                ...
                console.warn('[Supabase plugin] loadBranches error', { env, status: e?.response?.status, error: msg, response: e?.response?.data });
            }
        },
